...
# Fine-tune pi0.5 base on our collected real demonstrations with W&B logging.
# Download base first with: ./sh_scripts/download_lerobot_models.sh --set pi05
# Enable weak real-image augmentation with: REAL_IMAGE_AUGMENT=1 ./sh_scripts/train_pi05_real_full_wandb.sh
#
# Fine-tuning modes:
#   FINETUNE_MODE=full   Full fine-tuning (default)
#   FINETUNE_MODE=expert Train action expert/projections only
#   FINETUNE_MODE=lora   Train LoRA/PEFT adapters only
...
case "$FINETUNE_MODE" in
  full)
    ...
    ;;
  expert|action_expert|action-expert)
  ...
  lora|peft)
    ...
    ;;
  *)
    echo "FINETUNE_MODE must be one of: full, expert, lora. Got: $FINETUNE_MODE" >&2
    exit 1
    ;;
esac
...
BASE_POLICY="${BASE_POLICY:-./models/lerobot/pi05_base}"
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/train/pi05_real_${FINETUNE_MODE}_wandb_${RUN_ID}}"
JOB_NAME="${JOB_NAME:-pi05_real_${FINETUNE_MODE}_wandb_${RUN_ID}}"
...
PI05_USE_RELATIVE_ACTIONS="${PI05_USE_RELATIVE_ACTIONS:-false}"
USE_PI05_QUANTILES="${USE_PI05_QUANTILES:-1}"
PEFT_METHOD_TYPE="${PEFT_METHOD_TYPE:-LORA}"
PEFT_R="${PEFT_R:-16}"
PEFT_LORA_ALPHA="${PEFT_LORA_ALPHA:-32}"
PEFT_TARGET_MODULES="${PEFT_TARGET_MODULES:-}"
PEFT_FULL_TRAINING_MODULES="${PEFT_FULL_TRAINING_MODULES:-}"
...
DEFAULT_INPUT_FEATURES='{"observation.state":{"type":"STATE","shape":[7]},"observation.images.camera1":{"type":"VISUAL","shape":[3,224,224]},"observation.images.camera2":{"type":"VISUAL","shape":[3,224,224]}}'
...
if [ "$MERGE_DATASET" = "1" ]; then
  if [ "$FORCE_MERGE" = "1" ] || [ ! -f "$MERGED_DATASET_ROOT/meta/info.json" ]; then
    merge_args=(--task-data-root "$TASK_DATA_ROOT" --output-root "$MERGED_DATASET_ROOT" --repo-id "local/collected_demo_task1_2_3_4")
...
if [ "$ACTION_MODE" = "delta" ] && [ "$MAKE_DELTA_DATASET" = "1" ]; then
  if [ "$FORCE_DELTA" = "1" ] || [ ! -f "$DELTA_DATASET_ROOT/meta/info.json" ]; then
    delta_args=(--source-root "$MERGED_DATASET_ROOT" --output-root "$DELTA_DATASET_ROOT")
...
common_args=(
  ...
  --policy.train_expert_only="$POLICY_TRAIN_EXPERT_ONLY"
  --policy.use_relative_actions="$PI05_USE_RELATIVE_ACTIONS"
...
if [ "$FINETUNE_MODE" = "lora" ]; then
  common_args+=(
    --peft.method_type="$PEFT_METHOD_TYPE"
    --peft.r="$PEFT_R"
    --peft.lora_alpha="$PEFT_LORA_ALPHA"
  )
  if [ -n "$PEFT_TARGET_MODULES" ]; then
    common_args+=(--peft.target_modules="$PEFT_TARGET_MODULES")
  fi
  if [ -n "$PEFT_FULL_TRAINING_MODULES" ]; then
    common_args+=(--peft.full_training_modules="$PEFT_FULL_TRAINING_MODULES")
  fi
fi

if [ "$USE_PI05_QUANTILES" != "1" ]; then
  common_args+=(--policy.normalization_mapping='{"ACTION":"MEAN_STD","STATE":"MEAN_STD","VISUAL":"IDENTITY"}')
fi
...
echo "pi0.5 real-demo fine-tuning"
...
echo "  PI05_DTYPE=$PI05_DTYPE"
echo "  PI05_GRADIENT_CHECKPOINTING=$PI05_GRADIENT_CHECKPOINTING"
echo "  PI05_COMPILE_MODEL=$PI05_COMPILE_MODEL"
echo "  PI05_USE_RELATIVE_ACTIONS=$PI05_USE_RELATIVE_ACTIONS"
echo "  POLICY_TRAIN_EXPERT_ONLY=$POLICY_TRAIN_EXPERT_ONLY"
echo "  USE_PI05_QUANTILES=$USE_PI05_QUANTILES"
if [ "$FINETUNE_MODE" = "lora" ]; then
  echo "  PEFT_METHOD_TYPE=$PEFT_METHOD_TYPE"
  echo "  PEFT_R=$PEFT_R"
  echo "  PEFT_LORA_ALPHA=$PEFT_LORA_ALPHA"
  echo "  PEFT_TARGET_MODULES=$PEFT_TARGET_MODULES"
  echo "  PEFT_FULL_TRAINING_MODULES=$PEFT_FULL_TRAINING_MODULES"
fi
...
python - "$DATASET_REPO" "$DATASET_ROOT" "$BASE_POLICY" "$USE_PI05_QUANTILES" <<'PYCHECK'
import json
import sys
...
repo_id, dataset_root, base_policy, use_quantiles = sys.argv[1:5]
...
if use_quantiles == "1":
    ...
    for key in ("observation.state", "action"):
        if key not in stats or "q01" not in stats[key] or "q99" not in stats[key]:
            raise RuntimeError(
                f"pi0.5 quantile normalization needs q01/q99 stats for {key}. "
                "Run src/lerobot/scripts/augment_dataset_quantile_stats.py or set USE_PI05_QUANTILES=0."
            )
PYCHECK

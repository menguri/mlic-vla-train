...
find_latest_run_dir() {
  find outputs/train -maxdepth 1 -type d -name 'pi05_real_full_wandb_*' \
    | sort \
    | tail -n 1
}
...
if [ -z "$REAL_RUN_DIR" ]; then
  echo "No real pi0.5 run found under outputs/train." >&2
...
if [ -z "$POLICY_PATH" ]; then
  echo "No real pi0.5 checkpoint found under outputs/train." >&2
...
cat <<EOM
pi0.5 real remote inference server
  HOST=$HOST
  PORT=$PORT
  FPS=$FPS
